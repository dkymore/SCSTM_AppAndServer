...
from permission import login , logout , logup , isUser , auth
...
def setReserves():
    ...
    sql = """
    INSERT INTO `reserve` (`time`, `person`, `part`, `desp`) VALUES (%s, %s, %s, %s);
    """
    db = db_sql()
    db.insert_sql(sql,value=(time,person,part,desp))
